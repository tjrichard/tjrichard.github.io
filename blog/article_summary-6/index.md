...
## ⚡️ 요약
> 프로덕트 관리를 위해 프로덕트 선행 지표가 반드시 필요하다. 이 때 프로덕트 지표는 모든 것을 측정하는것이 아니라, 프로덕트를 더 나은 방향으로 만들기 위한 의사결정이나 액션을 할 수 있는 지표여야 한다.
AARRR, HEART, 북극성 지표 세 종류를 적절히 혼합해 프로덕트 지표의 예시를 만들어 보았다. 아래는 사용자의 각 단계 별 성공을 측정하기 위한 지표들이다.  
본인의 프로덕트에 맞는 지표를 설정하는 것은 정해져 있는것이 아니라 항상 변화해야 한다. 위 에시에서도 본인의 프로덕트와 잘 맞는 부분이 다를 것이다. 가장 임팩트가 큰 것부터 집중하되, 필요하다면 각 단계를 더 심층적으로 들어가 살필 수도 있을 것이다. 프로덕트 지표를 선정하는 것은 프로덕트가 사용자의 어떤 단계와 상호작용 하는지, 프로덕트의 비즈니스 목표가 어떤지, 처음 수집하고 있던 데이터 셋에서 더 변화해나갈 부분은 있는지 등을 고려해서 계속해서 반복 개선해야 한다.
